...
public class CardController {
    private final ICardInterface iCardInterface;

    public CardController(ICardInterface iCardInterface) {
        this.iCardInterface = iCardInterface;
    }
    ...
    @GetMapping(path = "/{userId}/{cardId}/{balanceId}", produces = {MediaType.APPLICATION_JSON_VALUE})
    public CardResponse getCardWithCardId(@PathVariable String userId, @PathVariable String cardId, @PathVariable String balanceId){
        ModelMapper mapper = new ModelMapper();

        CardDTO cardDTO = iCardInterface.getCardWithCardId(userId, cardId, balanceId);
